...
async def get_latest_post(db: Session = Depends(get_db),
    ...
    result_query = db.query(models.Post, func.count(models.Like.post_id).label("likes")).join(
        models.Like, models.Like.post_id == models.Post.id, isouter=True).group_by(models.Post.id)
    result = result_query.filter(models.Post.title.contains(search)).limit(limit).offset(skip).all()
    print(result)
    return result
...
async def get_posts(id: int, db: Session = Depends(get_db),
    ...
    post = db.query(models.Post, func.count(models.Like.post_id).label("likes")).join(
        models.Like, models.Post.id == models.Like.post_id).group_by(models.Post.id).filter(models.Post.id == id).first()
    if not post:
